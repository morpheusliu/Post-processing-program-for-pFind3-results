...
for file_name in file_list:
	...
	while True:
		...
		if not data_protein:
			break
		if len(arry_data_protein)>=2:	#这个条件是过滤掉pfind.protein文件尾部的筛选部分
			try:
				int(arry_data_protein[0])	#这个条件是筛选protein group对应的ac 因为这种ac在行起始的地方都有序号
			except:
				if types=='b':
	...
	while True:
		...
		for acsssss in arry_tem2:
			if os.path.exists('data_protein_group/'+acsssss):
				nnn+=1
		if nnn>1:	#进行unique peptide筛选 发文章是后已经将这里的原pt变量删除 减少了这部分筛选
			continue
		arry_all_site=[]
		for ac in arry_tem2:	#第一次匹配，这次进行leading的位点匹配
			for sitess in arry_sitess:
				if os.path.exists('data_protein_group/'+ac):
					...
					if sequence.count(arry_final_data[0])>1:	#在发文章的这个版本删除了原sc变量的筛选
						continue
					while True:
		...
		for acb in arry_all_site:	#leading匹配完了之后，根据leading匹配的结果，找到leading在pfind鉴定中的所有sameset，再次进行位点匹配
			...
			for acss in arry_data_acs:
				if acss:
					...
					file_library.close()
					sequence=sequence.replace('I','L')
					if sequence.count(arry_final_data[0])>1:	#文章发表的版本去掉了原sc变量
						continue
					while True:
						site=sequence.rfind(arry_final_data[0])
						if not site+1:
		...
		for ac_x in arry_file_format_match_site:
			...
			if leader_ac in data_format_match_site:
				...
				if leader_ac in arry_arry6_format_match_site:
					...
					print(ac_x,file=file_data5,end='')	#20200210更改 没有这一句 某些初级匹配但是没有最终匹配成功的数据项将被忽略造成数据丢失
			else:
	...
	while True:
		...
		for ac_xx in arry_file_match_site_plus:
			...
			if arry_final_list[0]+'_'+arry_final_list[1]==arry_data_match_site[6]:	#潜在危险 目前的写法只能适用于每行数据只有一个leading ac的情况 一旦使用ptm模式 会造成丢数据的危险 20200210
	...
	print('AC\tSite\tGenename\tModified sequences\tSpectum Count\tDescription\tRatio '+calculation+'\tInterference Score\tIdentification Score',file=file_result)
	while True:
		data_genename=file_genename.readline().replace('\n','')
		if not data_genename:
			break
		arry_data_genename=data_genename.split('\t')
		print(str(arry_data_genename[0])+'\t'+str(arry_data_genename[1])+'\t'+str(arry_data_genename[12])+'\t'+str(arry_data_genename[2])+'\t'+str(arry_data_genename[10])+'\t'+str(arry_data_genename[11])+'\t'+str(arry_data_genename[4])+'\t'+str(arry_data_genename[6])+'\t'+str(arry_data_genename[8]),file=file_result)
	file_genename.close()
	file_result.close()
